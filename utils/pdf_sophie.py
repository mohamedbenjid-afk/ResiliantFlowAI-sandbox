...
ctx attendu (voir pages/2_Sophie.py, TAB 3 — S3 RAPPORT HEBDOMADAIRE) :
    {
        ...
        "equipe_dispo":  list[dict]  # nom, prenom, disponibilite, heures_restantes
    }
"""
...
def _cover(story, s, ctx):
    ...
    data = [
        ["Machine surveillée", ctx.get("machine", "—"), "Date rapport", ctx["date_str"]],
        ["RUL estimé", f"{ctx.get('rul', '—')}h", "Statut", statut],
        ["Température", f"{ctx.get('temp', '—')}°C", "Vibration", f"{ctx.get('vib', '—')} mm/s"],
    ]
    ...
    h = hashlib.sha256(
        f"{ctx['reference']}{ctx['date_str']}{ctx.get('machine','')}".encode()
    ).hexdigest()[:32]
    sig = Table([[Paragraph(f"Empreinte numérique : {h.upper()}", s["small"])]], colWidths=[W - 4 * cm])
...
def _kpis(story, s, ctx):
    ...
    kpis = [
        ...
        (str(ctx["arrets_evites"]), "Arrêts évités", VERT_CLAIR),
        (str(ctx["n_ruptures"]), "Ruptures stock", ROUGE_CLAIR if ctx["n_ruptures"] else GRIS_C),
        (f"{ctx['n_dispos']}/{ctx['n_equipe']}", "Techniciens dispos", GRIS_C),
        (f"{ctx['cout_total']:,.0f} €", "Coût réalisé", GRIS_C),
    ]
...
def _interventions(story, s, ctx):
    ...
    hdr = [["Intervention", "Type", "Statut", "Date", "Technicien", "Durée", "Coût"]]
    rows = []
    for i in hist:
        ...
        rows.append([
            ...
            i.get("date", "—"),
            i.get("technicien", "—"),
            f"{i.get('duree_estimee', '—')}h",
            f"{cout:,.0f} €" if isinstance(cout, (int, float)) else "—",
        ])
    t = Table(hdr + rows, colWidths=[4.3 * cm, 2.3 * cm, 2.2 * cm, 2.2 * cm, 2.8 * cm, 1.6 * cm, 1.6 * cm])
    t.setStyle(TableStyle([
...
def _stock(story, s, ctx):
    ...
    hdr = [["Pièce", "Statut", "Stock actuel", "Stock minimum"]]
    rows = [[
        p.get("designation", "—"),
        p.get("statut_stock", "—"),
        str(p.get("stock_actuel", "—")),
        str(p.get("stock_minimum", "—")),
    ] for p in pieces]
    t = Table(hdr + rows, colWidths=[7 * cm, 4 * cm, 3.5 * cm, 3.5 * cm])
    ...
    for idx, p in enumerate(pieces, start=1):
        if p.get("statut_stock") == "Rupture":
            style.append(("BACKGROUND", (1, idx), (1, idx), ROUGE_CLAIR))
        elif p.get("statut_stock") == "Stock faible":
            style.append(("BACKGROUND", (1, idx), (1, idx), HexColor("#fef3c7")))
...
# ── CHARGE ÉQUIPE ────────────────────────────────────────────────────────────────
def _equipe(story, s, ctx):
    story.append(_sec_header("4. CHARGE ÉQUIPE MAINTENANCE", s))
    story.append(Spacer(1, 0.2 * cm))

    equipe = ctx.get("equipe_dispo") or []
    if not equipe:
        story.append(Paragraph("Aucune donnée d'équipe disponible.", s["body"]))
        story.append(Spacer(1, 0.3 * cm))
        return

    hdr = [["Technicien", "Disponibilité", "Heures restantes"]]
    rows = [[
        f"{t.get('prenom','')} {t.get('nom','')}".strip() or "—",
        t.get("disponibilite", "—"),
        f"{t.get('heures_restantes', '—')}h",
    ] for t in equipe]
    t = Table(hdr + rows, colWidths=[7 * cm, 5 * cm, 6 * cm])
    t.setStyle(TableStyle([
        ("BACKGROUND", (0, 0), (-1, 0), BLEU_MED),
        ("TEXTCOLOR", (0, 0), (-1, 0), white),
        ("FONTNAME", (0, 0), (-1, 0), "Helvetica-Bold"),
...
def generate_sophie_pdf(data: dict) -> bytes:
    ...
    ctx = {
        ...
        "equipe_dispo": data.get("equipe_dispo", []),
    }
